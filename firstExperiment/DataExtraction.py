...
if __name__ == '__main__':
    ...
    for direcotry in directories:
        ...
        files = os.listdir(sourcePath + direcotry + "/")
        for file in files:
            print("file: " + file)
            for word in sensor_keywords:
                if file != 'survey':
                    df = pd.read_csv(sourcePath + direcotry + "/" + file, delimiter="-", skipinitialspace=True, names=["DATE\TIME", "SENSOR", "VALUE"])
                    ...
                    if not os.path.isdir(path_dataset + '/processed/' + direcotry + '/' + w1 + '/'):
                        if not os.path.isdir(path_dataset + '/processed/' + direcotry + '/'):
                            os.mkdir(path_dataset + '/processed/' + direcotry + '/')
                        os.mkdir(path_dataset + '/processed/' + direcotry + '/' + w1 + '/')

                    df1.to_csv(path_dataset + '/processed/' + direcotry + '/' + w1 + '/' + word + '.csv')
        print()
    ...
    for user in users:
        ...
        for intake in intakes:
            ...
            for sensor in sensors:
                df = pd.read_csv(sensorsPath + "/" + sensor, delimiter=",", skipinitialspace=True)
